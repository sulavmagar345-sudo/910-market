...
export function generateDashboardReport(
  ...
  autoTable(doc, {
    ...
    body: [
      ['Total Revenue', `Rs. ${summary.totalRevenue.toLocaleString()}`, `${summary.revenueGrowth > 0 ? '+' : ''}${summary.revenueGrowth}%`],
      ...
      ['Average Order Value', `Rs. ${summary.averageOrderValue.toLocaleString()}`, `${summary.aovGrowth > 0 ? '+' : ''}${summary.aovGrowth}%`],
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['Today Revenue', `Rs. ${extendedSummary.todayRevenue.toLocaleString()}`, `${extendedSummary.todayRevenueGrowth > 0 ? '+' : ''}${extendedSummary.todayRevenueGrowth}%`],
    ],
    theme: 'grid',
    headStyles: { fillColor: [20, 61, 49], textColor: [255, 255, 255] },
    margin: { left: 20, right: 20 },
